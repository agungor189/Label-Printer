...
## 📤 Depo Planner JSON Export

Etiket listesinde **Depo JSON** butonu `packages-export.json` üretir. Bu dosya PDF değildir; Depo Planner uygulamasının import edeceği paket verisidir.
...
```json
{
  ...
  "packages": [
    {
      "packageId": "pkg-00001-dsdst-al-rd-1in-90d-1-4",
      "labelIndex": 1,
      "status": "unplaced",
      "placement": null,
      "sku": "DSDST-AL-RD-1IN-90D",
      "productName": "90° Dirsek",
...
Depo Planner tarafında `packages[*].placement` başlangıçta `null` kabul edilip raf gözüne yerleştirme sonrası doldurulabilir. Arama/highlight için `searchText`, `sku`, `productName`, `material`, `type`, `lot` ve `packageNo` alanları hazır gelir.
...
LABEL_RENDERER_API_KEY=uzun-rastgele-ortak-anahtar \
